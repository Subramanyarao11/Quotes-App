<template>
  <div
    ...
    "
    :style="[
    imageUrl ? { backgroundImage: `url(${imageUrl})` } : { backgroundImage: `url(https://images.unsplash.com/photo-1497561813398-8fcc7a37b567?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=870&q=80)` },
]"
...
export default {
  name: "Quotes",
   ...
   created () {
    ...
    methods: {
    async getImage() {
      this.imageUrl = await fetch('https://source.unsplash.com/random/?motivation')
        .then(res => res.url);
